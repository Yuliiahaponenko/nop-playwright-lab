...
Feature: Registered User Checkout
  ...
  Scenario: Complete checkout as registered user
    ...
    And I wait for any element using css "a[href='/customer/info']" to be visible
  ...
  @bdd2
  Scenario: Verify order in account after checkout
    Given I go to url "https://nop-qa.portnov.com"
    When I click element using css "a[href='/login']"
    And I fill element using css "#Email" with value "testuser@example.com"
    And I fill element using css "#Password" with value "Test123!"
    ...
    And I click element using css "button:has-text('Continue'):near(#payment-info-buttons-container)"
    When I click element using css "button:has-text('Confirm')"
    And I wait for any element using css ".order-number" to be visible
    When I go to url "https://nop-qa.portnov.com/customer/orders"
